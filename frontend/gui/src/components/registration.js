...
class RegistrationForm extends React.Component {
  ...
  validateForm = () => {
    ...
    usernameError = `${
      !this.state.credentials.username ? "Username can't be empty." : ""
    }`;

    // check password errors
    passwordError1 = `${
      !this.state.credentials.password ? "Password can't be empty." : ""
    }`;

    passwordError2 = `${
      this.state.credentials.password.length < 6
        ? "Password is too short. A minimum of six characters required."
        : ""
    }`;
    ...
    try {
      ...
      if (response_error) {
        ...
        if (usernameError || passwordError || confirmPasswordError) {
          ...
          this.setState({
            message: "Failed! Check URL or Internet connection.",
            loading: false,
          });
        }
      } else {
        this.setState({
          message: "Failed!You must start server.",
          loading: false,
          usernameError: "",
          passwordError: "",
          confirmPasswordError: "",
      ...
      <div>
        <Container className="center aligned">
          <Form onSubmit={this.handleSubmit}>
            ...
            <div className="success-register">{success}</div>
            ...
            <div className="register_errors">{usernameError}</div>
            ...
            <div className="register_errors">{passwordError}</div>
            ...
            <div className="register_errors">{confirmPasswordError}</div>
